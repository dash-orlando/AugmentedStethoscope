...
function startplaypush_Callback(hObject, ~, handles)
    ...
    try
        ...
        if singleradiostat == 1 && multipleradiostat == 0          
            ...
            set(handles.playstatus, 'String', 'Audio file will play once');
            play(audioplayobj); % play recording once        
        elseif singleradiostat == 0 && multipleradiostat == 1      
            disp(horzcat('Audio file will be played ',num2str(numofreps),' times'));
            set(handles.playstatus, 'String', horzcat('Audio file will be played ',num2str(numofreps),' times'));
            for i = 1:numofreps
                play(audioplayobj);
            end         
        end
...
function multiplaynum_Callback(hObject, ~, handles)
    ...
    % import variables from handles
        ...
        if isnan(numofreps) == 1
            disp('Error, input must only contain numbers!');
        elseif isnan(numofreps) == 0
