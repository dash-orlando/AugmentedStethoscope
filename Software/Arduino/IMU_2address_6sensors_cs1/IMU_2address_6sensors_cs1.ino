...
void setup() {
  ...
  for (int whichPair = 1; whichPair < 4; whichPair++) {
    ...
    if ( !high.begin() ) {
      ...
      Serial.println( (2 * whichPair - 1) );
      delay(500);
    };
    if ( !low.begin() ) {
      Serial.print( F("Failed to communicate with LSM9DS1: ") );
      Serial.println( (2 * whichPair) );
      delay(500);
    };

    //Calibration.
    Serial.println( F("Calibrating. Please wait."));
    double highx_hold, highy_hold, highz_hold, lowx_hold, lowy_hold, lowz_hold;
    int index_hi = (whichPair - 1) * 2;
    int index_lo = (2 * whichPair) - 1;
    for (int j = 0; j < CALIBRATION_INDEX ; j++) {

      delay(25);
      while ( !high.magAvailable() || !low.magAvailable() );
      ...
      //This function take the readings and reorients them appropriately within an array.
      //The global array has already has been declared 1 row per sensor, 3 columns per row; [6][3]
      orientRead(whichPair);

      //Taking the average, two sensors at a time.
      highx_hold += sens[index_hi][0];
      highy_hold += sens[index_hi][1];
      highz_hold += sens[index_hi][2];
      lowx_hold += sens[index_lo][0];
      lowx_hold += sens[index_lo][1];
      lowx_hold += sens[index_lo][2];

      if ( j == CALIBRATION_INDEX - 1) {
        cal[index_hi][0] = highx_hold / double(CALIBRATION_INDEX);
        cal[index_hi][1] = highy_hold / double(CALIBRATION_INDEX);
        cal[index_hi][2] = highz_hold / double(CALIBRATION_INDEX);
        cal[index_lo][0] = highx_hold / double(CALIBRATION_INDEX);
        cal[index_lo][1] = highy_hold / double(CALIBRATION_INDEX);
        cal[index_lo][2] = highz_hold / double(CALIBRATION_INDEX);

        //Purge temp. variables
        highx_hold = 0;
        highy_hold = 0;
        highz_hold = 0;
        lowx_hold = 0;
    ...
    //Print the calibration baseline reading, for each sensor.
    Serial.print( F("Calibration was succesful for sensor pair: ") );
    Serial.println(whichPair);

    //This one prints the odd sensor's values.
    Serial.print("Sensor Calibration: ");
    Serial.println(index_hi + 1);
    for (int k = 0; k < 3; k++) {
      Serial.print(cal[index_hi][k]);
      Serial.print(", ");
      if (k == 2) {
        Serial.println();
      };
    ...
    //This one prints the even sensor's values.
    Serial.print("Sensor Calibration: ");
    Serial.println(index_lo + 1);
    for (int k = 0; k < 3; k++) {
      Serial.print(cal[index_lo][k]);
      Serial.print(", ");
      if (k == 2) {
        Serial.println();
      };
...
void loop() {
  for (int whichPair = 1; whichPair < 4; whichPair++) {
    ...
    //Reorient the readings along the predefined C.S.
    orientRead(whichPair);
  };
