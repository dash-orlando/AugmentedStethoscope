...
void orientRead(int pair) {
  ...
  switch (pair) {
    ...
    case 3:
      //Sensor 5:
      sens[4][0] = ema_filter( double( high.calcMag(high.mx) ), 4, 0);
      sens[4][1] = ema_filter( double( high.calcMag(high.my) ), 4, 1);
      sens[4][2] = ema_filter( double( high.calcMag(high.mz) ), 4, 2);

      //Sensor 6:
      sens[5][0] = ema_filter( double( low.calcMag(low.mx) ), 5, 0);
      sens[5][1] = ema_filter( double( low.calcMag(low.my) ), 5, 1);
      sens[5][2] = ema_filter( double( low.calcMag(low.mz) ), 5, 2);
      break;
